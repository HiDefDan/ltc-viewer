...
- A rate/drop-frame indicator sits below the main readout, bottom-right: the classified frame rate (`23.98`/`24`/`25`/`29.97`/`30`) right-justified against the boundary between the two `FF` digit cells, and `dF` left-justified against that same boundary when the drop-frame bit is set. This is genuinely useful reference info for a professional LTC monitor, not a debug aid — LTC's bitstream carries no field for the true frame rate, only the drop-frame *convention* bit, so the rate is classified from the measured signal timing (not the bit) with hysteresis to prevent flicker between adjacent close rates (23.976/24 and 29.97/30 are ~0.1% apart), while `dF` is shown purely from the raw bit — independent of the classified rate, so an unusual combination (e.g. `30`+`dF`) is surfaced honestly as a real signal anomaly rather than hidden. A dim "88.88"/`dF` background is always visible (same treatment as the main "88.88.88.88." background) as a placeholder before lock; both disappear together on signal loss, tied to the same hold/gate as the main readout.
